...
public final class NQueensConfig {
    ...
    protected static void parseArgs(final String[] args) {
        ...
        {
            final int userInput = ap.getValue("-n", SIZE);
            SIZE = Math.max(1, Math.min(userInput, MAX_SOLUTIONS));
        }
        {
            final int userInput = ap.getValue("-t", THRESHOLD);
            THRESHOLD = Math.max(1, Math.min(userInput, MAX_SOLUTIONS));
        }
        NUM_WORKERS = ap.getValue("-w", NUM_WORKERS);
        SOLUTIONS_LIMIT = ap.getValue("-s", SOLUTIONS_LIMIT);
        {
            final int priority = ap.getValue("-p", PRIORITIES);
