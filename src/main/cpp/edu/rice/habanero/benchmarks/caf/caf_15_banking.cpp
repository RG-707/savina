...
behavior account(stateful_actor<account_state>* self, int /*id*/,
                 double balance_) {
  auto& s = self->state;
  s.balance = balance_;
  return {
...
#ifdef INFINITE
      self
        ->request(dest_account, infinite,
                  debit_msg{actor_cast<actor>(self), cm.amount})
        .await(
#elif HIGH_TIMEOUT
      self
        ->request(dest_account, seconds(6000),
                  debit_msg{actor_cast<actor>(self), cm.amount})
        .await(
#endif
...
behavior account(stateful_actor<account_state>* self, int /*id*/,
  ...
  return {
    ...
    [=](credit_msg& cm) {
      ...
      auto sender = actor_cast<actor>(self->current_sender());
      self->send(dest_account, debit_msg{actor_cast<actor>(self), cm.amount});
      self->become(keep_behavior, [=](reply_msg_atom) {
...
behavior teller(stateful_actor<teller_state>* self, int num_accounts,
                int num_bankings) {
  auto& s = self->state;
