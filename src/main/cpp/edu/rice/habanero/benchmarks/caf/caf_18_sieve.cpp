...
behavior prime_filter_actor_fun(stateful_actor<prime_filter_actor_state>* self,
  ...
  return {[=](long_box& candidate) mutable {
            auto& s = self->state;
            auto locally_prime = is_locally_prime(
              candidate.value, s.local_primes, 0, s.available_local_primes);
            if (locally_prime) {
          ...
          [=](string& x) {
            ...
            if (config::debug) {
              aout(self) << "Terminating prime actor for number "
                         << my_initial_prime << endl;
              ;
            }
            self->quit();
          }};
