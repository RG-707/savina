...
behavior consumer_actor(stateful_actor<consumer_actor_state>* self, int /*id*/,
                        actor manager) {
  auto& s = self->state;
  ...
  auto consume_data_item = [=](double data_to_consume) {
    auto& s = self->state;
    s.cons_item
      = process_item(s.cons_item + data_to_consume, config::cons_cost);
  };
  return {[=](data_item_msg& dm) {
            consume_data_item(dm.data);
            self->send(manager, s.consumer_available_message);
          },
          [=](consumer_exit_msg_atom) { self->quit(); }};
}
...
behavior producer_actor(stateful_actor<producer_actor_state>* self, int /*id*/,
                        actor manager, int num_items_to_produce) {
  auto produce_data = [=]() {
    auto& s = self->state;
...
behavior manager_actor(stateful_actor<manager_actor_state>* self,
                       int buffer_size, int num_producers, int num_consumers,
  ...
  for (int i = 0; i < num_producers; ++i) {
    s.producers.emplace_back(
      self->spawn(producer_actor, i, s.self, num_items_per_producer));
  }
  ...
  return {[=](data_item_msg& dm) {
            ...
            if (static_cast<int>(s.pending_data.size())
