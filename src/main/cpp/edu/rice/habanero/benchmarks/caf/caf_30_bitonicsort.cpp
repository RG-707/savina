...
behavior value_data_adapter_actor_fun(event_based_actor* self, int order_id,
  ...
  return {[=](value_msg& vm) {
          ...
          [=](data_msg& dm) { self->send(next_actor, dm); },
          [=](exit_msg_atom em) {
...
behavior data_value_adapter_actor_fun(event_based_actor* self,
                                      actor next_actor) {
  return {[=](value_msg& vm) { self->send(next_actor, vm); },
          [=](data_msg& dm) { self->send(next_actor, value_msg{dm.value}); },
          [=](exit_msg_atom em) {
            self->send(next_actor, em);
            self->quit();
          }};
...
behavior partition_bitonic_sequence_actor_fun(event_based_actor* self,
  ...
  return {[=](value_msg& vm) { self->send(splitter_actor, vm); },
          [=](exit_msg_atom em) {
...
behavior merge_stage_actor_fun(event_based_actor* self, int p, int n,
  ...
  return {[=](value_msg& vm) { self->send(forward_actor, vm); },
          [=](exit_msg_atom em) {
...
public:
  ...
  void run_iteration() override {
    ...
    auto kernel_actor = system.spawn(bitonic_sort_kernel_actor_fun, cfg_.n,
                                     true, adapter_actor);
